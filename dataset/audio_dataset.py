...
class AudioDatasets(torch.utils.data.Dataset):
    ...
    def __getitem__(self,idx):
        ...
        if int(data.shape[0]/self.sr) >= 5:
            data = data[:self.sr*5]
        elif int(data.shape[0]/self.sr) < 5:
            data = np.pad(data,pad_width=(0,self.sr*5-data.shape[0]), mode='constant', constant_values=0)
        data = data.reshape(1,self.sr*5)
